...
void Cartridge3EPlus::install(System& system)
{
    ...
    for (uInt32 i = 0x00; i < 0x40; i += (1 << System::S_PAGE_SHIFT))
...
void Cartridge3EPlus::bankRAMSlot(uInt16 bank)
{
    ...
    bool upper = (bank & BITMASK_LOWERUPPER) != 0x0; // is this the read or write port
  ...
  //     << "start=" << std::hex << start << ", end=" << end << endl << endl;
    for (uInt32 address = start; address <= end; address += (1 << System::S_PAGE_SHIFT))
    {
        if (upper)
...
void Cartridge3EPlus::bankROMSlot(uInt16 bank)
{
    ...
    uInt16 currentBank = bank & BIT_BANK_MASK;      // Wrap around/restrict to valid range
    bool upper = (bank & BITMASK_LOWERUPPER) != 0x0;  // is this the lower or upper 512b
