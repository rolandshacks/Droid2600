...
void EventHandler::toggleSAPortOrder()
{
    ...
    {
        mapStelladaptors("rl");
        myOSystem.showMessage("Stelladaptor ports right/left");
    }
    else
    {
        mapStelladaptors("lr");
        myOSystem.showMessage("Stelladaptor ports left/right");
    }
#endif
}
        ...
        else if (myState == S_EMULATE)
        {
            switch (key)
            {
                case KBDK_0:  // Ctrl-0 switches between mouse control modes
                    /*
                  if(myMouseControl)
                    myOSystem.showMessage(myMouseControl->next());
                    */
                    break;
                ...
                case KBDK_S:         // Ctrl-s saves properties to a file
                {
                    ...
                    if (out)
                    {
                        ...
                        myOSystem.showMessage("Properties saved");
                    }
                    else
                        myOSystem.showMessage("Error saving properties");
                    break;
                }
  ...
  // or need to be preprocessed before passing them on
    switch (event)
    {
      ...
      // If enabled, make sure 'impossible' joystick directions aren't allowed
        ...
        case Event::Fry:
            if (myUseCtrlKeyFlag) myFryingFlag = (state != 0);
            return;
        ...
        case Event::ConsoleColor:
            if (state)
            {
                ...
                myOSystem.showMessage("Color Mode");
            }
            break;
        ...
        case Event::ConsoleLeftDiffA:
            if (state)
            {
                ...
                myOSystem.showMessage("Left Difficulty A");
            }
            break;
        case Event::ConsoleLeftDiffB:
            if (state)
            {
                ...
                myOSystem.showMessage("Left Difficulty B");
            }
            break;
        ...
        case Event::ConsoleRightDiffA:
            if (state)
            {
                ...
                myOSystem.showMessage("Right Difficulty A");
            }
            break;
        case Event::ConsoleRightDiffB:
            if (state)
            {
                ...
                myOSystem.showMessage("Right Difficulty B");
            }
            break;
...
bool EventHandler::enterDebugMode()
{
    ...
    if (fbstatus != kSuccess)
    {
        ...
        if (fbstatus == kFailTooLarge)
            myOSystem.showMessage("Debugger window too large for screen",
                kBottomCenter, true);
        return false;
    }
    myOverlay->reStack();
    myOSystem.sound().mute(true);
#else
    myOSystem.showMessage("Debugger support not included");
#endif
