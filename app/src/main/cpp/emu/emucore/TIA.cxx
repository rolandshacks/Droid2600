...
void TIA::install(System& system, Device& device)
{
  // Remember which system I'm installed in
    ...
    for (uInt32 i = 0; i < 8192; i += (1 << System::S_PAGE_SHIFT))
        if ((i & 0x1080) == 0x0000)
...
inline void TIA::endFrame()
{
    ...
    {
        myFramerate = (myScanlineCountForLastFrame > 285 ? 15600.0f : 15720.0f) /
...
bool TIA::toggleBit(TIABit b, uInt8 mode)
{
  ...
  // Otherwise, flip the state
    bool on = (mode == 0 || mode == 1) ? (mode!=0) : !(myDisabledObjects & b);
...
bool TIA::toggleCollision(TIABit b, uInt8 mode)
{
    ...
    // If mode is 0 or 1, use it as a boolean (off or on)
    // Otherwise, flip the state
    bool on = (mode == 0 || mode == 1) ? (mode!=0) : !(enabled & b);
    if (on)  enabled |= b;
    else    enabled &= ~b;
...
bool TIA::toggleJitter(uInt8 mode)
{
  // If mode is 0 or 1, use it as a boolean (off or on)
  // Otherwise, flip the state
    bool on = (mode == 0 || mode == 1) ? (mode!=0) : myJitterEnabled = !myJitterEnabled;
    myJitterEnabled = on;
            ...
            else
            {
              // Update masks
                ...
                for (; myFramePointer < ending; ++myFramePointer, ++hpos)
                {
                    ...
                    colorIndex = myPriorityEncoder[hpos < 80 ? 0 : 1][enabled | myPlayfieldPriorityAndScore];
                    *myFramePointer = myColorPtr[colorIndex];
