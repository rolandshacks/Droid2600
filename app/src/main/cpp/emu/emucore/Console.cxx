...
void Console::togglePhosphor()
{
    ...
    if (phosphor == "YES")
    {
        ...
        enable = false;
        myOSystem.showMessage("Phosphor effect disabled");
    }
    else
    {
        myProperties.set(Display_Phosphor, "Yes");
        enable = true;
        myOSystem.showMessage("Phosphor effect enabled");
    }

    myOSystem.enablePhosphor(enable, blend);
}
...
void Console::changeYStart(int direction)
{
  ...
  if(direction == +1)       // increase YStart
  {
    if(ystart >= 64)
    {
      myOSystem.showMessage("YStart at maximum");
      return;
    }
    ystart++;
  }
  else if(direction == -1)  // decrease YStart
  {
    if(ystart == 0)
    {
      myOSystem.showMessage("YStart at minimum");
      return;
    }
    ystart--;
  }
  ...
  ostringstream val;
  val << ystart;
  myOSystem.showMessage("YStart " + val.str());
...
void Console::changeHeight(int direction)
{
  ...
  if(direction == +1)       // increase Height
  {
    ...
    {
      myOSystem.showMessage("Height at maximum");
      return;
    }
  }
  else if(direction == -1)  // decrease Height
  {
    ...
    if(height < 210)
    {
      myOSystem.showMessage("Height at minimum");
      return;
    }
  }
  else
  ...
  ostringstream val;
  val << height;
  myOSystem.showMessage("Height " + val.str());
...
void Console::toggleHMOVE() const
{
    if (myTIA->toggleHMOVEBlank())
        myOSystem.showMessage("HMOVE blanking enabled");
    else
        myOSystem.showMessage("HMOVE blanking disabled");
}
...
void Console::toggleFixedColors() const
{
    if (myTIA->toggleFixedColors())
        myOSystem.showMessage("Fixed debug colors enabled");
    else
        myOSystem.showMessage("Fixed debug colors disabled");
}
